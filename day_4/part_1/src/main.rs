...
fn main() -> std::io::Result<()> {
    ...
    let mut num_of_fully_contained_pairs = 0;
    ...
    for current_line_of_file in reader.lines() {
        ...
        let pair_two_range_contained_within_pair_one_range = pair_two_start >= pair_one_start && pair_two_finish <= pair_one_finish;
        let pair_one_range_contained_within_pair_two_range = pair_one_start >= pair_two_start && pair_one_finish <= pair_two_finish;

        if pair_two_range_contained_within_pair_one_range || pair_one_range_contained_within_pair_two_range {
            num_of_fully_contained_pairs += 1;
        }


    }
    println!("num_of_fully_contained_pairs: {}", num_of_fully_contained_pairs);
    Ok(())
}
